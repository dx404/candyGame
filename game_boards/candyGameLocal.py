...
#         }
    ...
    def initBoard(self, board, player):
        n = len(board)
        state = {}
        state['N'] = n
        state['player'] = player
        state['board'] = board
        state['vacantCount'] = n * n
    ...
    def run(self):
        ...
        while not state['finished']:
            t0 = time()
            self.counter = 0
            #x, y, robotExpScore = self.miniMax(state, 0)
            x, y, robotExpScore = self.alphaBeta(state, -100 * n * n, 100 * n * n, 0)
            t1 = time()
            state = self.drop(x, y, state)
            print state['player'], chr(65+y), (x+1), state['blueScore'], state['greenScore'], (t1 - t0), self.counter
...
if __name__=="__main__":
    candyGame = CandyGame("minmax", "minmax", 6)
    rawBoard = [
        [66, 76, 28, 66, 11, 9],
        [31, 39, 50, 8, 33, 14],
        [80, 76, 39, 59, 2, 48],
    ...
    # read board from command line specified files
    rawBoard = []
    f = open(sys.argv[1])
    for line in f:
        rawBoard.append([[int(x), 'V'] for x in line.split()])
    print rawBoard
    
    candyGame.initBoard(rawBoard, "blue")
    candyGame.run()
